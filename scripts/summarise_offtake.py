...
cf = cf.loc[:,((cf.columns.get_level_values(4).str.contains("google H2 Electrolysis"))|
            (cf.columns.get_level_values(4).str.contains("google H2 Store")))]
#%%
...
for volume in res.columns.levels[2]:
    ...
    caps = elec.xs((res_share, volume), level=[1,2], axis=1).xs("google", level=1).droplevel(0).reindex(wished_policies, level=0, axis=1).fillna(0)
    caps = caps.stack().reindex(wished_policies, axis=1).fillna(0).unstack()
    caps = caps.reindex(wished_order, level=1, axis=1)
    caps.rename(columns=rename_scenarios,
                        level=0,inplace=True)
    ...
    plot_duration_curve(cf, "google H2 Electrolysis", wished_policies, wished_order, volume,
                        name="electrolysis")
    plot_duration_curve(cf, "google H2 Store", wished_policies, wished_order, volume,
                        name="store")
#%%
# volume = "3200.0"
...
#                   override_component_attrs=override_component_attrs())
# htank = n.generators_t.p.loc[:,n.generators.bus=="google"].groupby(n.generators.carrier, axis=1).sum()[["onwind", "solar"]]
# htank = pd.concat([htank], keys=["htank"], axis=1)
# mtank = m.generators_t.p.loc[:,n.generators.bus=="google"].groupby(n.generators.carrier, axis=1).sum()[["onwind", "solar"]]
