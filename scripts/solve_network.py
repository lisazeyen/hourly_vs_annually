...
def DE_targets(n, snakemake):
    ...
    logger.info("Increasing electricity demand by factor %.2f", round(load_scale[year], ndigits=2))
    ...
    logger.info("Remove RES target and fix generation capacity for DE")
...
def capacity_target_constraint(n, snakemake):
    ...
    def cap_constraint(c, carrier_list, carrier):
        ...
        if rhs<0 or ext_i.empty:
            ...
            logger.info(f"scaling {carrier} by scale factor {scale_factor}.")
        else:
            logger.info(f"Add capacity constraint of {carrier} with {rhs} MW.")
...
def prepare_network(n, snakemake):
...
def solve_network(n, tech_palette):
    ...
    if result != "ok" or message != "optimal":
        ...
        solver_options["OptimalityTol"] = 1e-8
        result, message = n.optimize(
                extra_functionality=extra_functionality,
                solver_name=solver_name,
                solver_options=solver_options,
